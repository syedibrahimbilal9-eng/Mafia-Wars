// server.js
// Online Mafia game for exactly 6 players: 1 Mafia, 1 Detective, 1 Doctor, 3 Civilians.
...
io.on("connection", (socket) => {
  ...
  socket.on("joinRoom", ({ code, name }) => {
    ...
    if (room.players.length >= ROOM_SIZE) return socket.emit("errorMsg", "Room is full (6 players max).");
  ...
  socket.on("startGame", () => {
    ...
    const roles = ["mafia", "doctor", "detective", "civilian", "civilian", "civilian"];
